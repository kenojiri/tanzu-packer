...
pushd /mnt/datadisk/Bitnami
  ...
  cd argo-cd
  mkdir .imgpkg
  helm template nginx . --set volumePermissions.enabled=true --set redisWait.enabled=true --set dex.enabled=true | kbld -f - --imgpkg-lock-output .imgpkg/images.yml
  export CHART_VERSION=\$(yq .version Chart.yaml)
  export IMGPKG_REGISTRY_HOSTNAME_0="*.docker.io"
  export IMGPKG_REGISTRY_USERNAME_0="${DOCKERHUB_USERNAME}"
  export IMGPKG_REGISTRY_PASSWORD_0="${DOCKERHUB_PASSWORD}"
